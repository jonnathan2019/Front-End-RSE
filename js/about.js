...
var tabs_eva_repor = document.querySelectorAll(".about-repoter-info");
var tab_wraps_eva_repor = document.querySelectorAll(".contenido-about");

tabs_eva_repor.forEach(function (tab, tab_index) {
    tab.addEventListener("click", function () {
        tabs_eva_repor.forEach(function (tab) {
            tab.classList.remove("active");
        })
        tab.classList.add("active");

        tab_wraps_eva_repor.forEach(function (content, content_index) {
            if (content_index == tab_index) {
                content.style.display = "block";
            }
            else {
